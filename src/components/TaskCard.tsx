...
import { Truck, AlertCircle, CheckCircle, Camera, ChevronDown, ChevronUp, Phone, Building2, ArrowRight } from 'lucide-react';
...
export function TaskCard({ trip, onStartTask, onUploadPhoto, onMarkIncomplete, onMarkComplete, uploading }: TaskCardProps) {
  ...
  const packagePhotoUrl = trip.packagePhoto?.filePath
    ? `/${trip.packagePhoto.filePath.replace(/\\/g, '/')}`
    ...
    <div className={`bg-white rounded-xl shadow-sm border-l-4 ${stageConf.border} overflow-hidden mb-3`}>
      ...
      <div className="flex items-start justify-between p-4 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="flex items-center gap-2 ml-2 shrink-0">
          {packagePhotoUrl && (
            <img src={packagePhotoUrl} alt="Package" className="w-10 h-10 rounded-lg object-cover border border-gray-200" />
          )}
        ...
        <div className="px-4 pb-4 border-t border-gray-100 pt-3 space-y-3">
          ...
          {trip.remark && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-2.5 text-xs text-red-700">
              <span className="font-semibold">Reason: </span>{trip.remark}
            </div>
          )}
          ...
          {(trip.proofPhotos || []).length > 0 && (
            <div>
              <div className="text-xs font-semibold text-gray-500 mb-2">Proof Photos</div>
          ...
          {stage === 'Incomplete' && (
            <div className="flex items-center gap-2 text-red-500 text-sm font-semibold">
              <AlertCircle size={16} /> Marked incomplete
            </div>
          )}
        </div>
      )}
